...
export default function AirportInput({ label, value, onChange, resolved, onResolve, error }: AirportInputProps) {
  ...
  // "New York"을 치는 중 NEW 단계에서 New Orleans를 크게 주장하게 된다.
  ...
  const latestRef = useRef<{ q: string; rows: Airport[] }>({ q: "", rows: [] });
  /** 비행 중인 요청 — blur가 같은 질의를 다시 쏘지 않고 이것을 await한다. */
  const pendingRef = useRef<{ q: string; promise: Promise<Airport[]> } | null>(null);
  ...
  const runSearch = (q: string): Promise<Airport[]> => {
    const mySeq = ++seqRef.current;
    const promise = api.airports
      .search(q)
      .then((rows) => {
        ...
        setResults(rows);
        setActiveIndex(-1);
        // 해석만 한다 — 여기서 onChange를 부르면 스페이스 삭제 버그가 그대로 부활한다.
        if (IATA_CODE_RE.test(q)) {
  ...
  const handleBlur = () => {
    ...
    const q = raw.trim();
    // 빈 값에서는 어떤 경우에도 onChange를 부르지 않는다(부모의 touched 플래그가 켜지면 안 된다).
    if (!q) {
      setNoCommit(null);
      return;
    }
    ...
    void rowsPromise.then((rows) => {
      ...
      const decision = decideBlurCommit(raw, rows, { codeLength: 3, dismissed: dismissedRef.current });
  ...
  // 큰 글씨·인라인 이름은 "확정된 코드"일 때만 — 타이핑 중에는 크기가 흔들리지도, 엉뚱한 도시를
  // 크게 주장하지도 않는다. 타이핑 중 해석 결과는 아래 작은 라벨로만 힌트를 준다.
  // (편집 모드의 행은 city가 null이거나 빈 문자열일 수 있다.)
  const isCodeDisplay = resolved != null && committed && value.trim().toUpperCase() === resolved.iata;
  const inlineName = isCodeDisplay ? (resolved.city ?? "").trim() : "";
